...
# 어짜피 csv형태로 저장할꺼니까 헤더를 그냥 미리 정의한다.
header=",".join(["name", "age", "e-mail", "telephone", "sex"])+"\n"

for i in range(num_sample):
    name=random_name()

    # 결과물 파일에 이름 붙이기(F2 누른거랑 같음)
    filename = "personal_fake_info/" + str(i) + "_" + name + ".csv"

    # 파일 생성
    outfile = open(filename, 'w')
    
    # 이전 예제(TXT파일 1000개 생성)과 달리 헤더를 미리 정의
    outfile.write(header)
    
    outfile.write(name + ",")

    # 20이상 60이하 정수를 뽑아 나이를 설정
    # concat은 문자데이터만 되기 때문에 str()
    outfile.write(str(random.randint(20, 60)) + ",")

    # 미리 설정해둔 도메인 중에서 하나 뽑아 이메일 구성
    outfile.write(random_string(random.randint(5, 12)) + "@" + random.choice(domain_list) + ".com,")
